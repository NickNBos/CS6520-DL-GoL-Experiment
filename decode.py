...
def setup_dict():
    ...
    for index, item in enumerate(item_list):
        ...
        bitstring_map[item] = bitstring_rep
    ...
    for index, item in enumerate(item_list):
        ...
        y_map[y_code] = zero_string

# Automatically build the necessary maps when this module is loaded.
setup_dict()
...
def decode(x_string):
    ...
    x_string = x_string.replace('yy', y_map['yy'])

    for y_code, zero_string in y_map:
    ...
    for line in x_strings:
        ...
        for character in line:
            line_rep.append(bitstring_map[character])
...
def add_padding(x, new_size = WORLD_SIZE):
...
if __name__ == '__main__':
    ...
    for example in example_list:
        decoded_example = decode(example)
        resized_example = add_padding(decoded_example, 32)

        plt.figure(example, clear=True)
        visualize(resized_example)
        plt.show()
